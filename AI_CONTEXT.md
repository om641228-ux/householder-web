...
**2026-08-03 (текущая финальная версия, v17 — перевод выбранной страницы рядом с изображением в карточке)**
...
- ФИКС «перевод уезжает за правый край и обрезается»: корневая причина — .modal-image-section в CSS имеет flex:0 0 300px (узкая колонка слева от modal-info), внутри неё ряд [изображение+текст] не помещался. Решение: при наличии галереи секции выдаётся inline style flex:1 1 100% (inline перекрывает класс, CSS-файл не трогаем — однофайловые чеки остаются в старой раскладке); колонки внутри — изображение 3 1 380px / текст 2 1 300px с maxWidth:100% и overflowWrap:anywhere (длинные CUPS/URL)
