...
**2026-08-03 (текущая финальная версия, v18 — проверка перед статусом КОПИЯ: разные договоры больше не дубли)**
...
- РЕШЕНИЕ (frontend, поиск дубликатов): 1) документы совсем без идентичности (ни названия, ни даты, ни суммы) в группы не включаются; 2) внутри группы — проверка по СИЛЬНЫМ идентификаторам (contract_number, cups, invoice_number, supply_address): если у обоих поле заполнено и значения различаются (без частичного вхождения — «ALCOJORA RTB8B» ⊂ «ALCOJORA RTB8B N° B5, ADEJE» не конфликт) → документы разводятся в разные подгруппы, КОПИЯ не присваивается. Юнит-тест: разные контракты со скриншота разведены, настоящий дубль контракта (тот же № договора, адрес извлечён полнее) и дубль чека — по-прежнему ловятся
