...
- **Вкладка «🤝 CRM» (v32–v34), компонент CrmTab в App.js:** разделы 📅 Календарь / 📋 Задачи / 👥 Контрагенты / 📇 Контакты + сводка-счётчики (в работе / на подтверждении / просрочено / закрыто за месяц).
  ...
  - **Фотоотчёт (v35):** блок «📷 Фотоотчёт» в карточке задачи (секции «🕐 До выполнения» / «✅ После выполнения»): миниатюры 64px (клик → полноэкранный просмотр photoViewer), кнопка «📷» добавляет фото (multiple, сжатие compressImageFile до ~2 МБ), «✕» удаляет (с confirm). Тот же блок встроен в модалку «✅ Выполнена» — исполнитель сразу прикладывает фото результата. Бейдж «📷 n/m» (до/после) в карточке задачи в списке и календаре. Редактирование доступно постановщику и исполнителю, пока задача не закрыта; события photo/photo_del попадают в таймлайн. В локальном fallback-режиме фото хранятся как dataURL в задаче.
