...
**2026-08-12 (v35.2 — отступы шапки и строки настроек, компактность селекторов)**
...
- Frontend (App.css + apple-theme.css):
  1) `.App` — боковой padding увеличен с `16px` до `28px` — весь контент (шапка, список, тулбар) отодвинут от краёв окна.
  2) `.mini-header` — добавлен горизонтальный padding `4px` (был `0`), шапка не прижимается к внутренним краям контейнера.
  3) `.upload-toolbar` — gap уменьшен с `10px` до `6px`; кнопки тулбара (Фото, Выбрать файл, Распознать папку, Выписка банка) — padding уменьшен с `10px 18px` до `8px 14px`.
  4) `.toolbar-controls` — gap уменьшен с `6px` до `3px`; `.control-group.compact` — gap с `4px` до `2px`.
  5) Селекторы (Валюта, Тип, Подтип, Оплата, Объект, Режим) — label `font-size` `11px` → `10px`, select padding `6px 24px 6px 8px` → `5px 18px 5px 6px`, select `font-size` `12px` → `11px`. Строка настроек компактнее и не выходит за края при padding 28px.
- apple-theme.css: `.upload-toolbar, .toolbar-controls, .recognize-bar` gap переопределён с `10px` на `6px`.
- Метка сборки: «сборка 2026-08-12 · v35.2». Деплой: householder-web.
