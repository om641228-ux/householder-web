...
**2026-08-12 (v35.3 — отступы шапки, строки настроек, серая подложка, панель массовых действий в 2 строки)**
- Запрос пользователя: «Исправь шапку — она близко к боковым краям, сделай отступ. На втором скрине то же — отступ слева и справа, и уменьши строку настройки чтобы она не выходила за края. Сделай серую подложку везде на странице загрузки. Перенеси строку замены вниз — СМЕНИТЬ вниз во всю строку, умести меню, подстрой под общий стиль полукруглое».
- Frontend (App.css + App.js + apple-theme.css):
  1) `.App` — боковой padding `16px` → `40px` — весь контент отодвинут от краёв окна.
  2) `.mini-header` — padding `0` → `8px` по бокам.
  3) `.upload-section` — `background: transparent; border: none; box-shadow: none` — убран белый прямоугольник на странице загрузки.
  4) `.upload-toolbar` — padding `0 4px`, gap `6px`, кнопки `8px 14px`.
  5) `.toolbar-controls` — padding `8px 4px`, `flex-wrap: wrap` — селекторы переносятся при нехватке ширины.
  6) Селекторы загрузки — компактнее: label `10px`, select `5px 14px 5px 6px`, `font-size` `11px`.
  7) `.receipt-list`, `.filters-row` — padding `0 4px`.
  8) **Панель массовых действий** (App.js + App.css): убран inline-стиль с белым градиентом. Новые CSS-классы:
     - `.bulk-actions-panel` — прозрачный фон, flex column, gap 10px
     - `.bulk-actions-row` — flex row, gap 8px, wrap
     - `.bulk-actions-row-full` — нижняя строка во всю ширину
     - `.bulk-btn` — единый стиль пилюль (`border-radius: 999px`), padding `6px 14px`, варианты: danger (красный), success (зелёный), purple (фиолетовый), teal (бирюзовый), gray (серый)
     - `.bulk-select` — пилюля-селект (`border-radius: 999px`), стрелка справа
     - Верхняя строка: Выбрано, Удалить, Экспорт, Загрузить, Перераспознать, Перевести
     - Нижняя строка (во всю ширину): Сменить объект, Сменить тип, Сменить подтип, Сменить оплату, Сменить валюту, Сбросить, Показать копии
- apple-theme.css: `.upload-section` отделён от `.receipt-card` (у upload-section убраны фон/рамка/тень).
