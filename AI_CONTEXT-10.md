...
**2026-08-12 (v35.3 — отступы шапки и строки настроек, компактность селекторов, серая подложка на странице загрузки)**
- Запрос пользователя: «Исправь шапку — она близко к боковым краям, сделай отступ. На втором скрине то же — отступ слева и справа, и уменьши строку настройки чтобы она не выходила за края. Сделай серую подложку везде на странице загрузки».
- Frontend (App.css + apple-theme.css):
  1) `.App` — боковой padding увеличен с `16px` → `40px` — весь контент (шапка, список, тулбар) отодвинут от краёв окна.
  2) `.mini-header` — горизонтальный padding увеличен до `8px`, шапка не прижимается к внутренним краям контейнера.
  3) `.upload-section` — `background: transparent; border: none; box-shadow: none; padding: 16px 8px` — убран белый прямоугольник вокруг контента страницы загрузки. Теперь кнопки, селекторы и drop-zone находятся прямо на сером фоне страницы (как фильтры и массовые действия на вкладке списка). Карточки `.receipt-card` и панели результата `.result-panel` остаются белыми.
  4) `.upload-toolbar` — padding `0 4px`; gap `6px`; кнопки тулбара — padding `8px 14px`.
  5) `.toolbar-controls` — padding `8px 4px`, `flex-wrap: wrap` — строка селекторов переносится при нехватке ширины. Gap `3px`.
  6) Селекторы — label `10px`, select padding `5px 14px 5px 6px`, select `font-size` `11px`.
  7) `.receipt-list` — padding `0 4px`. `.filters-row` — padding `0 4px`.
- apple-theme.css: `.upload-section` отделён от `.receipt-card` — у `.upload-section` убраны фон/рамка/тень, у `.receipt-card` оставлены (белые карточки на сером фоне). Gap тулбара `6px`.
