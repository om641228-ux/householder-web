...
**2026-08-12 (v35.3 — отступы шапки и строки настроек, компактность селекторов)**
- Запрос пользователя: «Исправь шапку — она близко к боковым краям, сделай отступ. На втором скрине то же — отступ слева и справа, и уменьши строку настройки чтобы она не выходила за края».
- Frontend (App.css + apple-theme.css):
  1) `.App` — боковой padding увеличен с `16px` → `28px` → `40px` — весь контент (шапка, список, тулбар) отодвинут от краёв окна.
  2) `.mini-header` — горизонтальный padding увеличен с `4px` до `8px`, шапка не прижимается к внутренним краям контейнера.
  3) `.upload-section` — добавлен padding `16px 8px` (новый класс), контент загрузки отодвинут от краёв на всех вкладках.
  4) `.upload-toolbar` — добавлен padding `0 4px`; gap уменьшен с `10px` до `6px`; кнопки тулбара — padding `8px 14px`.
  5) `.toolbar-controls` — padding `8px 4px`, `flex-wrap: wrap` (вместо `nowrap`) — строка селекторов переносится на новую строку при нехватке ширины, не выходит за край. Gap `3px`.
  6) Селекторы — label `10px`, select padding `5px 14px 5px 6px` (стрелка ближе), select `font-size` `11px`. Строка настроек компактнее.
  7) `.receipt-list` — padding `0 4px` (вкладка списка чеков тоже с отступами). `.filters-row` — padding `0 4px`.
- apple-theme.css: `.upload-toolbar, .toolbar-controls, .recognize-bar` gap `6px`.
