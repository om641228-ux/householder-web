...
| POST | `/api/crm/counterparties/:id/files` | **Файлы контрагента (v36):** multipart, поле `files` (фото/видео/аудио, ≤100 МБ) → Storage папка `crm_cp/`; объект `{url, kind, name, ts, actor}` дописывается в attachments (jsonb). Любой авторизованный (командное пространство) |
