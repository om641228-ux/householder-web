...
## v96 (2026-08-27) — КРИТИЧЕСКИЙ фикс: слетали права на разделы
- Баг: в index.js ТРИ места трактовали tabs как «массив или null» — refreshUsersCache (~152), login (~311), POST /api/users (~472). Объект {tab:'full'|'read'|'none'} терялся → null → «всё открыто». Сохранённые ограничения слетали при сохранении/входе/рестарте.
- Добавлен normTabs(t): массив (старый формат) + объект с валидацией значений full/read/none. Применён во всех трёх местах.
- Health: v96-2026-08-27. Фронт не менялся. После деплоя: пересохранить права нужных пользователей в «👥 Доступ», пользователям — перелогиниться.

## v97 (2026-08-27) — выпадающие меню прав с галочками + видимость «чьи записи» по разделам
- tabs[k] теперь: строка full/read/none ИЛИ объект {l: уровень, v: ['*'|userId,...]} — normTabs валидирует оба.
- index.js: tabLevel читает v.l; explicitVis(user, tab): undefined=нет настройки / null=все / массив; применён в GET receipts (tab list), GET /api/crm (tab crm), GET cash-movements (tab cash — фильтр только при явной настройке).
- App.js UsersTab: кастомный дропдаун вместо select: уровни с ✓, для list/crm/cash блок галочек «Чьи записи видны» (Все + каждый пользователь); свои видны всегда. Состояние openTabDrop. Сводка в карточке пользователя умеет показывать объектные значения.
- Health: v97-2026-08-27. Сборка фронта v97.

## v98 (2026-08-27) — удалены встроенные user1…user10
- index.js: USERS = только admin; /api/users/names больше не добавляет user1..10. Health v98-2026-08-27.
- App.js: список «(встроенный)» в правах — только admin; тексты-подсказки обновлены. Сборка v98.
- ВАЖНО: вход по старым паролям user1..10 больше не работает — всем нужны учётки в app_users.

## v99 (2026-08-28) — один чек из нескольких файлов (вариант 1+4)
- Режим «📑 Один документ» существовал (multiPageMode='single' → recognizeDocumentPages → /api/upload-document-pages). Усилено:
...
## v103 (2026-08-28) — подсветка поиска в карточках
- HighlightText теперь мультисловный: запрос режется на слова (split(/\s+/)), regex-альтернация, подсвечивается каждое слово (совпадения — нечётные индексы parts после split с capture-группой).
- В карточке чека добавлена подсветка: дата/время, сумма, «N товаров», subtype (уже были: store_name, object, supply_address, owner).
- Только frontend. App.js md5 0887b7afa92dea81d490c5372921d814.

## v103.1 (2026-08-28) — усиление подсветки поиска
- mark получил className="hl-mark" + глобальный CSS `mark,.hl-mark{background:#ffeb3b !important;...}` в шапке — подсветка не перебивается никакими стилями.
- Логика v103 (мультисловный regex) подтверждена тестом: split с capture-группой, совпадения на нечётных индексах.
- App.js md5 см. ниже. Если подсветки нет — проверить футер: должно быть v103.1 (старый бандл в кэше).

## v104 (2026-08-29) — флажок «полное совпадение» в поиске
- Стейт searchExact рядом с searchQuery; чекбокс «полное совпадение» справа от строки поиска.
- Вкл: фильтр allText.includes(вся фраза); выкл: по-прежнему AND по словам.
- HighlightText читает модульный флаг HL_EXACT (выставляется в рендере App) — вкл: подсвечивает фразу целиком, выкл: каждое слово.
- App.js md5 0ae7d3ea84e6adddd03a0c81f3906245. Только frontend.

## v104.1 (2026-08-29) — фикс «полного совпадения»: 0 результатов
...
## v106.3 (2026-08-29) — мобильная версия: пакет правок 2
- MOBILE_CSS переписан: все правила ТОЛЬКО через body.mobile-view (раньше @media по ширине ломал принудительный десктоп — «не сразу переключается»).
- Переключатель версий — сегмент 📱|🖥 с подсветкой активной (auto: активный эффективный подсвечен светлым; повторный клик по активному → авто). cycleUiMode остался только в шторке «Ещё».
- «Выбор модели» → компактная кнопка «AI». Mac OCR в шапке — значок «⌘ OCR» (не путать с 🖥).
- Вкладка Загрузка на мобильном: скрыты (hide-mobile) Выписки банка, Бэкап, В облако, Восстановить, все селекторы (toolbar-controls).
- Нижняя навигация: все разделы (Фактуры, Загрузка, Cash, CRM, Анализ, Налоги, Доки, Чат, Доступ, Журнал, Ещё) + Apple-прокрутка (overflow-x momentum, scroll-snap, без скроллбара).
- App.js md5 33d5cc897c17fd12a9b6154a216613ad.

## v106.4 (2026-08-29) — мобильная версия: список чеков
- Фильтры Год/Месяц/Тип/Подтип/Объект/Разница скрыты на мобильном (обёртка hide-mobile + display:contents).
- Поиск + чекбокс «точно» (бывш. «полное совпадение») в одну строку (nowrap, minWidth:0).
- Кнопка «🔄 Обновить» скрыта на мобильном.
- Сортировка — горизонтальная прокрутка (nowrap, overflow-x auto, flexShrink:0).
- Таймлайн месяцев (dateRailGroups): на мобильном — горизонтальная полоса fixed над нижней навигацией (bottom: calc(64px + safe-area)), overflow-x auto.
- App.js md5 ниже.

## v106.5 (2026-08-30) — мобильная версия: убраны лишние bulk-кнопки
- На мобильном (hide-mobile) скрыты в панели выбора чеков:
  1) «Все (Excel+Фото+Текст)» select + «📊 Excel (все)» + «🔍 Дубликаты»
  2) «⬇ Загрузить ▾», «🔄 Перераспознать», «🌐 Перевести»
...
## v106.8 (2026-08-31) — мобильная: убран ⌘ OCR, Загрузка первая
- Из мобильной шапки удалена кнопка «⌘ OCR» (осталась только ⚡ Распознать); на десктопе Mac OCR остаётся в recognize-bar.
- Нижняя навигация и порядок свайпа: Загрузка → Фактуры → Cash → … (было Фактуры первыми).
- md5 efa057fbe0b172a1c8f69bc28a8a7ce3. Только frontend.

## v106.9 (2026-08-31) — кнопки распознавания в первой строке + бейдж модели в шапке
- «Распознать и сохранить» и «🖥 Локально (Mac OCR)» перенесены из recognize-bar в первую строку upload-toolbar (перед селекторами Валюта/Тип/…), компактные стили; recognize-bar остался только с меткой сборки.
- В шапке (на ВСЕХ вкладках) рядом с AI/переключателем версии — кликабельный бейдж выбранной модели (provider-badge + displayName), клик открывает модал выбора модели.
- md5 31e0ef4165dd52fdb8b2146ca1b0b702. Только frontend.

## v106.10 (2026-08-31) — убрана кнопка AI, метка сборки в шапке
- Кнопка «AI» удалена из шапки во всех версиях (выбор модели — клик по бейджу модели в шапке).
- Метка «сборка 2026-08-31 · v106.10 · Mac OCR: … ⚙» перенесена из recognize-bar в первую строку шапки (перед именем пользователя); пустой recognize-bar удалён из разметки.
- md5 ниже. Только frontend.

## v106.11 (2026-08-31) — мобильная: скрыты кнопки распознавания в тулбаре
- «Распознать и сохранить» (recognize-main-btn) и «Локально (Mac OCR)» получили hide-mobile — на мобильном остаётся только «⚡ Распознать» в шапке.
- md5 см. md5sum. Только frontend.

## v106.12 (2026-09-01) — мобильная шапка: минимализм
- «⚡ Распознать» → только значок ⚡ (при работе — ⚡ N%).
- Метка в шапке на мобильном: «2026-08-31 · v106.12» (без слова «сборка», без Mac OCR и ⚙); на десктопе полный текст.
- Бейдж модели: шрифт серый тонкий 11px как у метки сборки.
- «Выйти» на мобильном → толстая стрелка ⬆ (title «Выйти»). Всё — на всех вкладках (шапка общая).

## v106.13 (2026-09-01) — мобильная: CRM и шапка
- Версия на мобильном — второй строкой под шапкой (справа, 10px); в первой строке остались переключатель, модель, Dima, ⬆.
- CRM: скрыты заголовок «🤝 CRM», инфо-текст «Данные CRM хранятся…» и сводка 4 карточек (В работе/…/Закрыто за месяц); кнопки Календарь/Задачи/Контрагенты/Контакты/Новая задача остались. isMobileView прокинут пропом в CrmTab.
- «Выбрать все на странице» → «Все» на мобильном.
...
## ext v1.18.0
- Фото товара: запасные источники og:image → самое большое фото галереи; относительные URL → абсолютные (фикс «битых» фото Worten).
- MPN (№ производителя): JSON-LD mpn → gtin13 → микроразметка/itemprop → текст «MPN/Ref./Modelo/Nº de artículo» на странице.
- Инцидент: manifest.json был случайно обнулён при патче — восстановлен полностью.

## ext v1.18.1 + v168-sql
- MPN regex ловил «Refresca tu hogar» (меню Worten) → везде «resca». Фикс: \b-границы + обязательная цифра в номере; gtin13 как запасной MPN. SQL v168: mpn=null для resca/без цифр/<4 символов у Worten.

## ext v1.18.2 + v169-sql
- Worten: № производителя теперь = поле «Modelo» из характеристик страницы (не EAN/gtin13). SQL v169: обнулить mpn = чистый EAN (12–14 цифр) у Worten. После — «Обновить старые цены» пачкой.

## ext v1.18.3
- Modelo-экстракция Worten переделана: v1.18.2 regex не матчил (\s* съедал \n перед значением). Теперь DOM-поиск ячейки «Modelo» → соседняя ячейка/[class*=value]; запасной regex «Modelo\nзначение». Перезапуск: режим «Обновить старые цены».
// --- v169 ---
v169 (2026-09-09): backend — GET /api/parse/catalog/stats?site= {total, withPrice, processed, remaining, lastPriceAt}; фильтр ?brand= в /api/parse/catalog и /export; /api/parse/logs фильтрует по колонке site ИЛИ url (or-запрос); insert в parse_logs из mercadona/sync получил site. frontend — инфо-панель магазина над каталогом (всего/спарсено/с ценой/осталось + последняя цена + статус расширения) и прогресс-бар withPrice/total, автообновление 15с; клик по бренду в таблице = фильтр, повторный клик снимает (кнопка ✕ 🏷 у строки результатов, учитывается в export); вёрстка таблицы: название товара обрезается до 3 строк (maxWidth 340, line-clamp), колонка ✏️ sticky справа — кнопка редактирования всегда видна; журнал парсинга показывает записи конкретного магазина.
v170 (2026-09-09): backend — sitemap .xml.gz распаковывается zlib.gunzipSync (PrestaShop); артикул: добавлен паттерн /(\d{1,7})-slug.html (am[4]); build v170. frontend — новый магазин TuTrebol (www.tutrebol.es, sitemap https://www.tutrebol.es/sitemap_index_shop_1.xml — индекс из 9 gzip-файлов по ~2000 товаров).
ext v1.19.0: попап → БОКОВАЯ ПАНЕЛЬ Chrome (sidePanel permission + side_panel.default_path=popup.html, убран action.default_popup, setPanelBehavior openPanelOnActionClick); popup.html резиновая ширина; в селектор магазинов добавлен TuTrebol.
v171 (2026-09-09): tutrebol не парсился — сервер Railway в timeout 45s (IP блокируется/трафик режется). Backend: общий upsertSitemapXml(xml,url), новый POST /api/parse/ext-sitemap {url, xml}; серверный фетч sitemap — 120с + 1 retry. Ext v1.20.0: onMessageExternal cmd 'sync-sitemap' — браузер качает sitemap со своего IP, gunzip через DecompressionStream, POST XML на сервер. Frontend: syncSitemap при ошибке сервера автоматически пробует через расширение (extSend sync-sitemap), результат в чипе «(через браузер)».
ext v1.20.1 (2026-09-09): extractOnPage — PrestaShop (TuTrebol): цена из [itemprop=price]/.current-price/#our_price_display, бренд из строки «Marca» (соседняя ячейка), mpn из «cod. NNNN», gtin из «EAN»; onMessage type 'status' → {running,last,progressAt}. popup.js: живой опрос статуса каждые 2с, popup.html: индикатор ▶ ИДЁТ СБОР/■ + стилизованный блок статуса.
ext v1.21.0 (2026-09-09): ПАРАЛЛЕЛЬНЫЙ сбор по магазинам — activeRuns/stopReq/stopAllQ вместо одного running для очередей цен (run()); каждая очередь помечает прогресс тегом [WORTEN]/[TUTREBOL]/[LERAY...]; стоп останавливает всё, новый start снимает общий стоп; status → {running, queues}. Разделы/бренды остались эксклюзивными. Капча останавливает только свою очередь.
v172 (2026-09-09): MediaMarkt — бренд и № производителя из НАЗВАНИЯ: deriveBrandMpn(name, site): сегмент после « - » (или всё название для mediamarkt/tutrebol), brand=первое слово, mpn=до первой запятой/скобки, в mpn обязательна цифра. Применяется при sitemap-синке (все магазины) + endpoint POST /api/parse/catalog/derive-brand-mpn {site} (дозаполнение, до 100k) + кнопка «🏷 Бренд/№ из названий» в инструментах каталога. ext v1.21.1: те же правила из h1 страницы при сборе цен.

## v173 (2026-09-09) — MediaMarkt починен + прогресс-бар по магазинам
**Проблема 1: «не парсится MediaMarkt»** — корневая причина найдена: canarias.mediamarkt.es это Shopify, товарные URL вида `/products/slug` БЕЗ числового артикула, а upsertSitemapXml с v167 отбрасывал все URL без артикула (`if (!am) return null`) → синк sitemap давал 0 товаров (9 шт в базе — из старого парсинга разделов). Фикс index.js: паттерн `/\/products\/[a-z0-9-]+$/i` = товар даже без артикула (article=null). Sitemap-индекс MediaMarkt: ~30 файлов `sitemap_products_N.xml?from=...&to=...` по ~1000 товаров, с сервера доступен (0.9с).
**Проблема 2: цены MediaMarkt** — у Shopify-темы нет JSON-LD/itemprop; цена в `div.price .regular-price` (вложенные `.price__integer` + `.price__decimal`), старая в `div.price .compare-at-price` («89,- €»). Related-товары используют `.content_price` — их НЕ трогаем. Ext v1.22.0: dedicated-блок в extractOnPage; визуальный детектор v1.12 на MediaMarkt больше НЕ переопределяет найденную цену (он цеплял related-товары → мусор типа −50 EUR при цене 5.29).
**Проблема 3: прогресс-бар показывал чужой магазин** — ext v1.22.0: `lastBySite` (тег `[WORTEN]`→{text,at}) в ответах status; App.js pollExtStatus берёт запись только текущего магазина (stag из CAT_STORE.host), иначе «расширение собирает другой магазин (активных очередей: N)». 
**Бонус:** кнопка «⬇⬇ Синхронизировать ВСЕ (N)» над списком файлов индекса — последовательный синк всех subs с паузой 1.5с (App.js syncAllSubs).
**Параллельность:** очереди разных магазинов работают одновременно (v1.21.0) — можно запустить все 4-5 магазинов из одной панели по очереди или из нескольких окон.
Файлы: App.js+index.js v173 (deploy copies), leroy-price-extension v1.22.0 (zip).

## v174 (2026-09-10) + ext v1.22.1 — магазин Chafiras, Mercadona в попапе, CDATA в sitemap
- Chafiras.com = PrestaShop БЕЗ www (chafiras.com). Sitemap: /1_es_0_sitemap.xml (4.6MB, 6092 URL, ~5509 товаров, доступен с сервера). URL товара `/cat/176-slug-EAN13.html` → article = EAN13 (первый regex). Названия БЕЗ бренда-модели → deriveBrandMpn по h1 не применять.
- parseSitemapXml + subs-индекс: поддержка CDATA в loc/image:loc/image:title (chafiras оборачивает всё в CDATA). Без фикса парсилось 0.
- Ext: бренд Chafiras из текста описания «Marca: Brinox.» (regex, игнор «No brand»); цена — PrestaShop-блок (itemprop=price content) уже работал.
- Popup: добавлены Mercadona (tienda.mercadona.es) и Chafiras в селектор магазинов. host_permissions <all_urls> — вкладки chafiras открываются без изменений манифеста.

## v175 (2026-09-10) + ext v1.23.0 — антизависание + информативность
**Причина «10 минут ничего не меняется»**: fetch к серверу (fetchQueue, ext-price) был БЕЗ таймаута — повисший запрос к Railway вешал очередь навсегда, последний прогресс «⏳ Непрерывный сбор запущен…». Также зависшая вкладка/инъекция могла повесить collectOne.
Фиксы background.js: fetchT() (AbortController, 25с) для fetchQueue и ext-price; per-item watchdog — Promise.race(collectOne, 90с) → failReason 'timeout-90s'; при старте пачки прогресс «⏳ Запрашиваю очередь у сервера…».
Popup v1.23.0: runind показывает активные очереди («▶ ИДЁТ СБОР: TUTREBOL + WORTEN»); новый блок #queues — по каждому магазину последняя строка + «обновлено HH:MM:SS (N с назад)», красное предупреждение если живой очереди нет обновлений 2+ мин.
App.js v175: в баннере «🧩 Расширение работает» — «обновлено HH:MM:SS (N с назад)» и предупреждение о зависании. Бэкенд не менялся (остался v174).

## ext v1.24.0 (2026-09-10) — жёсткая валидация цен («каша» LM)
**Баг**: на страницах LM с вариантами (Estor «+24 Opciones») визуальный детектор v1.12 сканировал ВСЮ страницу и брал: цену из рассрочки «20 €/mes» (текст «Paga a plazos»), старую цену 269.05 из панели вариантов, % из чужого бейджа → строки «20 EUR / 269.05 / −2% / −249.05» при реальных 122,20 € / 201,95 / −39%.
**Фикс background.js**: (1) __visualPrice пропускает элементы внутри carousel/slider/recommend/related/option/variant/plazo/financ-блоков и тексты с «mes/plazo/cuota/a plazos»; (2) эталон __prePrice (JSON-LD/спец-блоки) — визуальная цена, расходящаяся с эталоном >2%, принимается ТОЛЬКО как скидка от эталона (старая≈эталон и % сходится ±3пп), иначе игнорируется; (3) финальная нормализация: % и € ВСЕГДА пересчитываются из пары цен; старая = цена/(1−%/100) если есть только бейдж; скидка <1% → сбрасывается.
**Чистка мусорных строк (SQL в Supabase)**: update parse_products set price=null, price_original=null, discount_pct=null, discount_abs=null where price_original is not null and price is not null and discount_pct is not null and abs(abs(discount_pct)-(1-price/price_original)*100)>3; — вернёт их в очередь «без цены».

## ext v1.24.1 (2026-09-10) — двухпроходный визуальный детектор
**Баг после v1.24.0**: главная цена верная (134 €), но старая/процент неверные (136.73/−2%): бейдж «−2 %» из промо-баннера перезаписывал настоящий «−39 %» (брался ПОСЛЕДНИЙ бейдж со всей страницы), а зачёркнутая 221,50 не находилась → нормализация вычисляла old=134/(1−0.02)=136.73.
**Фикс**: __visualPrice переписан на 2 прохода — (1) главная цена по странице с запретными зонами; (2) ценовой блок = предки элемента цены (до 6 уровней, контейнер с ≥2 символов €/%, <400 симв.) и ТОЛЬКО внутри него ищутся бейдж −N %, бейдж −N € и зачёркнутая цена. Первый найденный бейдж в блоке побеждает. jsdom-тесты: 134/221.50/−39% ✓; только бейдж → old=цена/(1−%) ✓; без скидки → чисто ✓.

## ext v1.25.0 (2026-09-10) — «интеллектуальный» сбор цен: стейт-JSON + блоковый визуал
Проблема 1.24.1: на части LM-страниц (JS-рендер) бейдж/зачёркнутая не находились → старая/процент пустые.
Решение (3 независимых источника, сверка): (1) СТЕЙТ-JSON страницы — все inline-скрипты сканируются на пары «price/previousPrice|originalPrice|pvp|…» (+ discountPercentage рядом), кандидат выбирается ПОСЛЕ визуала по ближайшей цене (защита от JSON рекомендаций); (2) визуальный двухпроходный (блок цены); (3) JSON-LD/meta. Приоритет: стейт, согласованный с визуальной ценой ±2%. Дополнительно в ценовом блоке незачёркнутая цена БОЛЬШЕ главной = старая (LM бьёт классом без line-through). Нормализация % и € из пары цен — всегда.
jsdom-тесты: полный LM-кейс 85.65/141.60/−39.5 ✓; только стейт (без видимой цены) ✓; только бейдж → old=цена/(1−%) ✓; чужой JSON рекомендаций игнорируется ✓.

## ext v1.26.0 (2026-09-10) — финальный редизайн источников цен
**Диагноз последнего бага** (price=60, old=128.6, −53.3% при реальных 128,60/212,60/−39%): на странице с вариантами JSON-LD и стейт-JSON содержали ДРУГОЙ вариант (60 €); проверка v1.24 отвергла правильную видимую цену 128,60; стейт подтянул «старую» 128.6 от чужого варианта. Плюс тройка склеивалась между попытками (d={...d,...nd}).
**Финальные правила**: (1) ВИДИМАЯ главная цена из ценового блока = истина для «Цена»; JSON-LD/стейт НИКОГДА её не переопределяют. (2) Старая/% — из того же блока; если нет — из стейт-кандидата, цена которого СОВПАДАЕТ с видимой ±2% (иначе чужой вариант); если и там нет — JSON-LD-цена выше найденной = «без скидки». (3) Тройка цена/старая/%/%€ мержится АТОМАРНО из одной попытки. (4) Нормализация % и € из пары цен всегда.
Тесты jsdom: D=сегодняшний кейс ✓ (128.6/212.6/−39.5), E=JSON-LD регулярная как старая ✓, F=без скидки ✓, + все кейсы 1.25 ✓.
Чистка: update parse_products set price=null, price_original=null, discount_pct=null, discount_abs=null where site='www.leroymerlin.es' and price_at >= '2026-09-10 22:00';

## v1.27.0 (2026-09-10) — LM: Ref-якорь + фильтр рассрочки
Проблема: строка 86176487 сохранялась как 10 EUR/104.1/−90.4% (10 = «3 x 10 €» из блока PayPal-рассрочки; 104.1/−90.4% — JSON-LD другого варианта «+ 24 Opciones»).
Исправления в background.js:
1. __visualPrice(root, anchorEl): для leroymerlin.* якорь = элемент /^Ref\.?\s*\d{5,}/i; от якоря подъём ≤8 уровней до контейнера с «€» (<2500 симв.) — поиск цен только внутри него.
2. SKIP_CTX: предок (≤3 ур., ≤250 симв.) с /paga a plazos|cuotas?|financia|paypal|plazos?|\d+ € ?\/? ?mes/i → элемент не цена товара. ВАЖНО: НЕ пропускать, если в тексте предка есть «%» или ≥2 «€» (это ценовой блок, а не финансы) — иначе .product-col с текстом «Paga a plazos» глушил ВСЕ элементы (баг найден dbg3.mjs).
3. collectOne: для LM цикл ожидания рендера (8×1с) пока в контейнере Ref не появится «€».
Тесты jsdom: test_lm (85.65/141.6/39.5), test_lm2 A/B/C, test_lm3 D/E/F, test_lm4 G (121/200/39.5) — все PASS.
Очистка мусора в БД:
  update parse_products set price=null, price_original=null, discount_pct=null, discount_abs=null where site='www.leroymerlin.es' and price_at >= '2026-09-10 22:00';

## v1.27.1 (2026-09-10) — LM: парная сверка цена×старая×бейдж
Проблема: страница 86180369 (−39%, 239,40 → 144,85) сохранилась как 85/144.85/−41.3% — 85 € = цена конфигуратора («desde 85 €», шрифт крупнее), а РЕАЛЬНАЯ цена 144,85 была ошибочно принята за СТАРУЮ (правило «n > price*1.02»).
Исправления в __visualPrice (extractOnPage):
1. readPrice: отсекает «desde …»/«from …» — цены конфигуратора «от N €».
2. Собираются ВСЕ кандидаты цен (cands) и ВСЕ зачёркнутые (olds).
3. ПАРНАЯ сверка: при наличии бейджа −N % перебираются все пары (кандидат × зачёркнутая), выбирается пара с |(1−p/o)·100 − N| ≤ 4 п.п. (мин. отклонение, при равенстве — крупнее шрифт). Одиночная сверка (только price против old) НЕ срабатывала: 85 против неправильного old=144.85 давала отклонение 2,3 ≤ 4.
Тесты: A–H + новый I («85 €» крупным без слова desde) — все PASS: 144.85/239.40/39.
Очистка БД:
  update parse_products set price=null, price_original=null, discount_pct=null, discount_abs=null where site='www.leroymerlin.es' and price_at >= '2026-09-10 22:00';

## v1.27.2 (2026-09-10) — LM: бейдж «−N %» как якорь ценового блока
Проблема: 86182031 (−39% / 318,50 / 192,70) сохранилась как 70/192.7/−63.7% — «70 €» с большим шрифтом вне ценового блока стало главной ценой, реальная 192,70 ушла в «старую», бейдж не нашёлся (подъём от чужого элемента), % вычисленный.
Решение: НОВЫЙ приоритетный проход в __visualPrice ДО поиска по шрифту — ищем элементы вида «−N %» (badge), от каждого поднимаемся ≤6 уровней до контейнера с зачёркнутой ценой + ≥2 «€» (<400 симв.), внутри принимаем ТОЛЬКО пару (цена, зачёркнутая), где |(1−p/o)·100 − N| ≤ 4 п.п. Нашли → готово (done), старые проходы пропускаются. Нет бейджа → прежняя логика (проходы 1–2 + парная сверка v1.27.1).
Тесты A–J все PASS (J = сегодняшний кейс: 192.7/318.5/39 при ловушке «70 €» крупным шрифтом).
Очистка БД:
  update parse_products set price=null, price_original=null, discount_pct=null, discount_abs=null where site='www.leroymerlin.es' and price_at >= '2026-09-10 22:00';

## v176 (2026-09-11) — вкладки «Чеки» и «Предметы» (старт модуля предметов)
Идея пользователя: база домашних предметов — после распознавания фото (название/MPN) предмет заносится в хранилище дома, привязывается к чеку, ссылке на базу магазина; поиск похожих предметов по магазинным базам с ценами.
Шаг 1 (этот релиз): в навигацию после зоны кнопки модели добавлены две вкладки — «🧾 Чеки» (открывает список чеков, вкладка по умолчанию) и «📦 Предметы» (activeTab='items'). Контент «Предметов» = ПОЛНАЯ копия вкладки чеков (условие activeTab === 'list' || 'items'), дальше будем отключать лишнее. Также: мобильная нижняя навигация (📦), свайп-порядок, TAB_LABELS (items: '📦 Предметы'), автоперевод карточки работает и в items.

## v177 (2026-09-11) — верхний переключатель проекта Чеки/Предметы
Пользователь поправил v176: «Чеки» и «Предметы» — НЕ кнопки в общем меню, а ВЕРХНИЙ переключатель (в строке кнопки модели, где красная линия на скрине). Реализация: state appMode ('checks'|'items', default 'checks'); переключатель-сегмент «🧾 Чеки | 📦 Предметы» в шапке сразу после кнопки модели. При переходе в «Предметы» открывается activeTab='items' (клон проекта: полное меню + контент вкладки чеков как стартовая точка); в меню появляется пометка «📦 Проект «Предметы» — клон». Кнопки v176 из вкладочного меню и мобильного bottomnav откачены. Дальше: отключать лишнее в меню предметов и строить модуль (фото→AI→карточка предмета→привязка к чеку/магазину→похожие с ценами).

---

## v178 (2026-09-11) — модуль «📦 Предметы»: фото → AI → база дома → поиск по магазинам (шаги 1–4)

Пользователь прислал 9 тестовых фото предметов (набор метчиков, тестер розеток NJTY T 003, магнитный угольник, калибр 3D-печать, держатель бит T15, кусачки TOLSEN/GodHand SPN-120, шестигранник Amazon Basics) и утвердил схему: наименование + % схожести, производитель, номер производителя.

**SQL (один раз, SQL Editor):** `supabase-migration-v178-home-items.sql` — таблица `home_items`:
- `name_ru`, `name_original`, `category`, `confidence` (0..1, % схожести наименования);
- `brand` + `brand_confidence`, `mpn` + `mpn_confidence` (номер производителя);
- `ai_model`, `ai_raw` (отладка), `receipt_id` (привязка к чеку — следующие шаги), `photo_url`, `edited`, `notes`, `user_id`, триггер `updated_at`, RLS policy `true` (как остальные таблицы — доступ через backend).

**Backend (index.js, build v178-2026-09-11, feature 'home-items'):**
- `buildItemPrompt()` — отдельный промпт идентификации предмета: JSON {name_ru, name_original, category, confidence, brand, brand_confidence, mpn, mpn_confidence}; жёсткое правило «не читается → null, НЕ выдумывай»; размер («2 мм», «T15») — НЕ mpn.
- `parseItemJson()` — парсер (НЕ чековый parseAIResponse): извлечение {…}, confidence 0..1 или 0..100 → нормализация к 0..1, очистка «null/none/—».
- `recognizeItemGemini()` / `recognizeItemOpenAICompat()` / `recognizeItemWithFallback()` — та же цепочка, что у чеков: Gemini (все кандидаты) → OpenRouter → GitHub → Mistral → Kimi (k3: max_completion_tokens + reasoning_effort low).
- Эндпоинты (все requireAuth, токен в ?token=):
  - `POST /api/items/recognize` (multipart, поле image) → AI → фото в Storage (папка items/{userId}) → INSERT в home_items → {item}. Если таблицы нет → {missing:true} с подсказкой про миграцию.
  - `GET /api/items?q=` — список (поиск по name_ru/name_original/brand/mpn), missing → {items:[], missing:true}.
  - `PUT /api/items/:id` — правка полей, ставит `edited=true`.
  - `DELETE /api/items/:id` — admin/manager.
  - `GET /api/items/:id/similar` — похожие в parse_products по убыванию точности: 1) точный MPN; 2) brand + слова названия (≤3 слов ≥3 букв); 3) слова русского названия (≥4 букв). Дедуп по site|url, поле `match_by` (mpn/brand+name/name/name_ru), до 30 строк, сортировка по цене.

**Frontend (App.js v178):**
- Вкладка «items» БОЛЬШЕ НЕ клон чеков — заменена на панель предметов (условие `(activeTab === 'list' || activeTab === 'items')` → `activeTab === 'list'`). Это и есть начало «отключения» клона.
- Панель: кнопка «📷 Загрузить фото предмета» (input file, capture=environment для камеры телефона), поиск по предметам, баннер «таблица не создана» (missing), сетка карточек.
- Карточка: фото (клик → оригинал), название + бейдж % (≥90 зелёный, ≥70 жёлтый, <70 красный), ориг. название, категория, 🏭 бренд + %, 🔢 MPN + %, дата + модель; кнопки: ✏️ правка (инлайн-форма всех полей), 🛒 «Найти в магазинах» (результаты similar под карточкой: фото, название, тип матча, сайт, цена, зачёркнутая старая, −%), 🗑 удаление.
- `loadItems()` подключён в верхний переключатель appMode и в gotoTab('items').

**Деплой:** `householder-deploy/frontend/src/App.js` + `householder-deploy/backend/index.js` (md5 совпадают). Порядок: 1) SQL в Supabase; 2) backend на Railway; 3) frontend на GitHub.

**Следующие шаги модуля:** привязка предмета к чеку (receipt_id UI), массовая загрузка фото, сохранение результатов поиска в сам предмет, фильтры по категориям.

## v179 (2026-09-11) — Режим «Предметы»: своё меню (Загрузка + 🔧 Tools), фикс правки

Только App.js (бэкенд без изменений, v178). md5 App.js = f3d5a7b23784c027c6007f8fb33b3957 (= deploy/frontend/src/App.js). Бэкап до патча: App.js.bak-v178.

Что сделано (по запросу пользователя):
1. Убрана оранжевая плашка «Проект «Предметы» — клон…» из tabs-inline. Выбранная кнопка режима (Чеки/Предметы) и так подсвечивается синим (#0071e3).
2. В режиме «Предметы» меню = только «Загрузка» + «🔧 Tools». Все кнопки чекового режима (Фактуры, Анализ, Налоги, Cash, CRM, Документы, Связи, Парсинг, Цены, Чат, Доступ, Журнал) обёрнуты в `{appMode !== 'items' && (<>…</>)}` — десктоп (tabs-inline), мобильный bottomnav (включая «Ещё») и лист «Ещё» (пункты чеков; «Выбор модели AI» и режим интерфейса остаются).
3. Вкладка панели предметов переименована 'items' → 'tools': панель `{activeTab === 'tools' && …}`, gotoTab, TAB_LABELS (`tools: '🔧 Tools'`), переключатель режимов items → setActiveTab('tools'). mobileTabsOrder: 'items'→'tools' (только при appMode==='items') + фильтр: в режиме предметов свайп только между upload/tools.
4. Окно загрузки чеков `{activeTab === 'upload' && (` теперь `{appMode === 'checks' && activeTab === 'upload' && (`. В режиме «Предметы» на вкладке «Загрузка» — своё окно: 📷 фото → recognizeItemPhoto → карточка в home_items (логика как у чеков). Мобильная кнопка ⚡ в шапке — только appMode==='checks'.
5. Автоперевод raw_text_ru снова только во вкладке 'list'.
6. ФИКС скриншотной ошибки «Ошибка: Нет полей для обновления»: форма правки предмета открывалась с пустым itemEditForm, PUT уходил без полей → 400. Теперь onClick «✏️ Правка» предзаполняет форму значениями карточки (name_ru, name_original, brand, mpn, category, notes).

Деплой: frontend → GitHub (frontend/src/App.js). Бэкенд деплоить не нужно (v178 уже с /api/items/*).

## v180 (2026-09-11) — Предметы: вкладка «Загрузка» как у чеков, поиск по-испански (name_es), новый переключатель

md5 App.js = см. выше (deploy/frontend/src/App.js идентичен), index.js = e664b88b03db63b400542fa11ff6973b (deploy/backend/index.js идентичен). Бэкапы: App.js.bak-v179, index.js.bak-v178.

1. Переключатель Чеки/Предметы: убран объединяющий бордер-контейнер, кнопки разнесены (gap 8), у каждой своя рамка-пилюля; выбранная — сплошная синяя заливка #0071e3 (как «Загрузить фото предмета»).
2. Вкладка «Загрузка» в режиме «Предметы» — полная копия UX чеков: тулбар (📷 Фото/Камера, 📁 Выбрать файл multiple, ⚡ Распознать и сохранить (N)) + drop-zone «Перетащите фото предмета сюда» с превью, навигацией ◀/▶ по файлам и «✕ Очистить». Пакетное распознавание: recognizeItemFiles() шлёт файлы последовательно в /api/items/recognize, ошибки собираются в itemError. Стейт: itemFiles, itemFilesIdx; функции addItemFiles/itemDrop.
3. Поиск по магазинам исправлен: раньше брал слова name_original (там часто только бренд «Stanley FatMax» → матчей нет). Теперь:
   - AI-промпт возвращает name_es (испанское название: «alicates de corte diagonal»); parseItemJson/insert/PUT-whitelist/поиск списка — + name_es.
   - /api/items/:id/similar: 1) MPN exact; 2) name_es слова (стоп-слова de/del/la/con…, нормализация диакритики, OR-запрос до 120 кандидатов, JS-скоринг: +2 за исп. слово, +1 за латинское из name_original, +3 бренд, +5 MPN; порог ≥2, сортировка score→price) match_by='name_es'; 3) русские слова (fallback).
   - Фронт: карточка показывает «🇪🇸 name_es», форма правки + поле «Название (исп.)», метка матча «🇪🇸назв.».
4. SQL: supabase-migration-v180-home-items-name-es.sql — alter table home_items add column if not exists name_es text. Если миграция не выполнена, insert отработает без name_es (graceful retry в бэкенде).
5. Деплой: SQL в Supabase → бэкенд index.js на Railway → фронт App.js на GitHub.

Дальше (roadmap): визуальный поиск «картинка по картинке» — эмбеддинги фото предмета против фото parse_products (image-embedding модель, pgvector колонка, кандидаты из name_es-поиска → переранжирование по cosine similarity).

## v181 (2026-09-11) — Визуальный поиск «картинка по картинке» для предметов

md5 App.js = 8ae4c4ed688e275a756b4423400a8975, index.js = 76310e6ed09c18c6fcedc9cef53f5ed1 (deploy-копии идентичны). Бэкап: index.js.bak-v180.

Двухстадийная схема (без pgvector — индексировать весь каталог не нужно):
- Стадия 1 (кандидаты): как /similar — MPN exact + name_es-скоринг, но только строки parse_products с фото (image не null, абсолютный URL), топ-12 по score.
- Стадия 2 (vision-ранжирование): скачиваем фото предмета (photo_url) + фото кандидатов (axios, 10s таймаут, ≤10MB, только image/*), отправляем ОДНИМ запросом в vision-модель (№0 = предмет, №1..№12 = каталог) → JSON [{n, score}] (score ≥ 0.5, топ-10). Fallback-цепочка как у распознавания: Gemini (все кандидаты) → OpenRouter → GitHub → Mistral → Kimi (k3-особенности те же).
- Новое: buildVisualRankPrompt, parseVisualRankJson (нормализация 0..1/0..100), downloadImageBuffer, rankImagesGemini (inlineData parts), rankImagesOpenAICompat (image_url parts), rankImagesWithFallback, эндпоинт GET /api/items/:id/similar-visual → {results: [{...product, visual_score, match_by:'visual'}], candidates, model}. Пишется в журнал («Предметы» / «визуальный поиск»).
- Фронт: кнопка «🖼 По фото» на карточке (рядом с «🛒 Найти в магазинах»), стейт itemVisual, результаты — голубые строки с бейджем «🖼 N%» (≥85% зелёный, ≥65% жёлтый) + сайт + цена; шапка блока показывает число сравненных кандидатов и модель.
- SQL-миграций нет. Деплой: backend index.js → Railway, frontend App.js → GitHub.

## v182 (2026-09-11) — Предметы: папка, прогресс-бар, честная модель, Tools(N), антимусор в поиске, дубликаты

md5 App.js = 980c87a830c232a4fc0990dfc5ac0c34, index.js = c46f2905ad31c904bd2608025cfe8641. Бэкапы: App.js.bak-v181, index.js.bak-v181.

Фронт (App.js):
1. Вкладка «Загрузка» предметов: + «📁 Распознать папку» (input webkitdirectory, image-only фильтр в addItemFiles); строка «Модель: <activeModelDisplay> (смена — в шапке)».
2. Прогресс-бар пакетного распознавания: стейт itemProgress {done,total,name}, панель с «⚡ Распознаю i/N — имя файла» и синей полосой.
3. Фронт шлёт fd.append('model', selectedModel) в /api/items/recognize (одиночный и пакетный).
4. Кнопки Tools (десктоп+моб.) показывают счётчик: «🔧 Tools (N)» по itemsList.length.
5. Карточка: блок визуального поиска (itemVisual) перенесён ВЫШЕ текстового (itemSimilar); в бейдже схожести метка «✓×2» если результат подтверждён обоими проходами; метка «🔁 Дубликат» (it.dup) рядом с названием.

Бэкенд (index.js):
1. POST /api/items/recognize читает req.body.model: recognizeItemPreferred() (формат как у чеков: gemini-* / kimi-kimi-k3 → kimi + 'kimi-k3', и т.п.); при сбое выбранной — recognizeItemWithFallback. recognizeItemOpenAICompat принимает modelOverride.
2. Текстовый similar: стоп-слова + juego/set/pack/kit/profesional/universal/piezas/unidades; отсев мусора — нужно ≥2 исп. слова ИЛИ бренд+слово ИЛИ точный MPN (раньше хватало одного слова → «ROT…», «BARQ…»).
3. Визуальный similar-visual: усиленный промпт (точный тип предмета, тонкогубцы≠плоскогубцы≠кусачки, одиночный≠набор, сомнение→не включать, +0.2 за читаемый бренд); ДВА независимых прохода rankImagesWithFallback, консенсус: найден обоими → avg score + verified=true; одним → score×0.8; порог 0.4. В ответе visual_verified.
4. GET /api/items: метки дубликатов — совпадает mpn (нормализованный) или бренд+name_es/name_original/name_ru; помечаются ОБЕ карточки (поле dup).
5. Деплой: backend → Railway, frontend → GitHub. SQL не нужен.

## v183 (2026-09-11) — Предметы: визуальный поиск выбранной моделью, усложнённый промпт, MPN-гарда

md5 App.js = 3d0d13a5004a577abef7c7ed37fa4720, index.js = e3bbff1f375437e03ec38eb941abab59. Бэкапы: App.js.bak-v182, index.js.bak-v182.

1. ФИКС «выбрано Kimi, а работает Gemini»: распознавание с v182 честное (карточка показывала kimi-kimi-k3), но /similar-visual игнорировал выбор. Теперь фронт шлёт ?model=selectedModel в /similar и /similar-visual; rankImagesPreferred() (gemini-*/openrouter-*/github-*/mistral-*/kimi-*), при сбое выбранной — fallback. Оба прохода консенсуса используют выбранную модель.
2. Промпт распознавания усложнён (8 шагов): осмотр всех сторон (жало/губки/шарнир/торец/наклейки), сначала ТОЧНЫЙ тип («отвёртка короткая stubby под биты», не «отвёртка»), name_es 2–4 содержательных слова без set/kit/profesional; brand только с корпуса, не угадывать по цвету; MPN — только коды с буквами+цифрами или явные Ref/Art/Mod, короткие чисто-числовые («350», «200») = размер → null; mpn_confidence < 0.7 → mpn: null.
3. MPN-гарда в поисках: /similar и /similar-visual используют item.mpn только если длина ≥4, есть буква и (mpn_confidence ≥ 0.7 или не задана) — иначе «350» матчил клавиатуры. Применено и к точному матчу, и к +5 в скоринге.
4. Деплой: backend → Railway, frontend → GitHub. SQL не нужен.

## v184 (2026-09-11) — Исследование vision-моделей: цепочки обновлены

md5 App.js = c1e0ebd7c940cd7bee006810ced91709, index.js = ff02533b7efdc758d7758d6b823c8f16. Бэкап: index.js.bak-v183.

Выводы исследования (веб, сентябрь 2026):
- GitHub Models ЗАКРЫТ 30.07.2026 (playground, inference API, BYOK) → провайдер 'github' убран из цепочек предметов (recognizeItemWithFallback, rankImagesWithFallback) и из селектора моделей. В чековой цепочке пока оставлен (там свои fallbackIds) — TODO: почистить и там.
- Gemini: актуальные vision — gemini-3.5-flash, gemini-3.1-pro-preview, gemini-3-flash-preview; 1.5/2.0 сняты. GEMINI_FALLBACK_CANDIDATES = [3.5-flash, 3.1-pro-preview, 3-flash-preview, 2.5-flash, 2.5-pro].
- OpenRouter :free — Qwen2.5-VL делистнуты; vision free: gemma-4-31b/26b, nemotron-nano-12b-v2-vl, nemotron-3-nano-omni-30b (vl+reasoning), openrouter/free. defaultModel → gemma-4-31b-it:free.
- Mistral: pixtral-12b-2409 deprecated → fallback pixtral-large-latest (124B); дефолт mistral-small-latest.
- Kimi K3: нативный vision, MMMU-Pro 81.6 — фронтир, отлично подходит для распознавания предметов И визуального сравнения; API: reasoning_effort low/high/max, без temperature, base64-only картинки — наши вызовы уже так и шлют.
- /similar: +10 score за ПОЛНУЮ фразу name_es в названии товара (v184).
- Селектор фронта: GitHub-модели и Qwen :free удалены, добавлены Nemotron Omni VL, OpenRouter Free, Pixtral Large; «Gemini 3.1 Pro Preview ★ vision» помечен рекомендуемым.
Рекомендация пользователю: для распознавания и «🖼 По фото» выбирать Gemini 3.1 Pro Preview или Kimi K3; бюджетно — Gemini 3.5 Flash.

## ext v1.27.3 (2026-09-11) — ФИКС кривых цен Leroy (везде 129.99/216.99)

Файлы: leroy-price-extension/background.js (бэкап background.js.bak-1.27.2), manifest.json → 1.27.3. Deploy-копии синхронизированы.

Причина бага (не AI и не промпт — DOM-парсер): когда карточка товара схлопывалась до ссылки, __visualPrice поднимался к предкам (до 3 уровней, контейнер ≤1200 символов и ≤4 ссылок) и брал самую крупную по шрифту цену — ей оказывалась цена промо-блока/соседнего товара, одинаковая для всех карточек.

Фикс:
1. __visualPrice(root, anchorEl) — ценовой элемент учитывается только если его ближайшая товарная ссылка (a[href*=".html"]) — это ссылка ЭТОЙ карточки; элементы внутри ссылок на другие товары пропускаются.
2. Подъём к предкам строже: ≤600 символов и ≤2 товарных ссылок, и тоже с anchorEl.
3. Страничная страховка: если одна пара (price, price_original) встречается у ≥5 товаров и ≥40% страницы — такие цены зануляются (_price_dropped); товары без цены и так уходят в needPrice → дочитка со страниц товаров.
4. SQL-чистка уже записанного мусора: supabase-cleanup-leroy-bad-prices.sql (SELECT для просмотра застрявших пар + UPDATE по паре 129.99/216.99).

Пользователю: переустановить расширение (chrome://extensions → обновить), выполнить SQL, перезапустить синк разделов LM.

## ext v1.27.4 + web v185 (2026-09-11) — ФИКС цены со страницы товара LM, колонка «Дата загрузки» вперёд

md5 App.js = 0fdf3fe4befa9e8acea59f5368206347, background.js = 288e2d8a947008f9662ac9087e9f3e2d (deploy-копии идентичны). Manifest ext → 1.27.4.

Причина «129.99» на странице товара (реальная цена 9,29 €): (а) JSON-LD цена перезаписывалась ЛЮБЫМ блоком Product на странице — включая рекомендации/сопутку; (б) если якорь «Ref. NNNNN» не находил компактный блок (<2500 символов с €), визуальный детектор брал крупнейшую цену со ВСЕЙ страницы (промо-баннер).
Фикс background.js:
1. JSON-LD: ref товара из URL (-(\d{5,}).html); цена принимается только из Product, чей sku/productID/mpn содержит ref; после подтверждённого совпадения (__ldMatched) чужие Product цену не перезаписывают.
2. __visualPrice теперь возвращает res.anchored (удалось ли сузить область до блока якоря).
3. Если визуал НЕ заякорен и расходится с JSON-LD ценой >2% — верим JSON-LD (баннер отброшен).
Фронт v185: колонка «Дата загрузки» (last_seen, dd.mm.yy hh:mm; tooltip: первый парсинг + время цены) перенесена сразу после «Раздел» — видна без горизонтального скролла; старая позиция перед ✏️ убрана. Бэкенд /api/parse/catalog отдаёт select('*') — last_seen/price_at/first_seen приходят.
Пользователю: обновить расширение (1.27.4), задеплоить frontend v185, выполнить cleanup SQL из v1.27.3, перезапустить синк.

## ext v1.27.5 (12.09.2026)
- Баг: баннер «Envío gratis en pedidos +29€» читался как цена товара (напр. набор Torx → 99/29 EUR).
- Фикс (DOM-парсер, НЕ промпт): фильтры `envío|gratis|pedidos|entrega|recogida|entregar` и `+\d` добавлены в ОБЕ функции __visualPrice — listing (строка ~650, цикл по текстам) и product page (строка ~87, readPrice). Первая порция патча попала только в listing, добито вручную.
- manifest.json → 1.27.5; деплой-копия householder-deploy/leroy-price-extension/ синхронизирована (md5 0548ea6351d013a2e2a6fb2e6a47f3fa); zip leroy-price-extension-v1.27.5.zip (d38815f4a7ab419fab5bf0f06c931304).
- Файл supabase-wipe-leroy-herramientas.sql: SELECT-предпросмотр + два варианта (DELETE раздела / обнуление цен) для site=www.leroymerlin.es, category ilike 'Herramientas%' or url like '%/productos/herramientas/%'.

## ext v1.27.6 (12.09.2026)
- Панель: <select id="site"> заменён на чекбоксы .sitecb (мультивыбор «как в Excel»): можно отметить 1, 2, любые магазины; ни одной галки = все. Каждый отмеченный магазин стартует своей параллельной очередью (сообщение 'start' на каждый site; архитектура 'q:<site>' уже это поддерживает).
- popup.js: сохранение/восстановление sites[] (миграция со старого site), start() шлёт start на каждый выбранный сайт; background.js: расписание (alarms) повторяет мультивыбор.
- manifest → 1.27.6; деплой-копия синхронизирована; zip leroy-price-extension-v1.27.6.zip.

## v186 (12.09.2026) — Tools: выбор, сортировка, тайм-шкала, место хранения
- Tools: сортировка (дата распознавания ↓↑/название/бренд/%), карточек на странице (12/24/48/96/все + пагинация ◀▶), логарифмическая тайм-шкала справа (месяц YYYY-MM + log-бар, клик = фильтр).
- Выбор галками (чекбокс на карточке + «Выбрать все на странице»); пакетные действия: 🗑 Удалить, 🔄 Перераспознать (новый POST /api/items/:id/rerecognize — качает photo_url, recognize с выбранной моделью, edited=false), 💾 Выгрузить в папку (File System Access API showDirectoryPicker: фото + items.json; fallback — скачивание).
- «Найти в магазинах ▾» — выпадающее меню: Все / Leroy / Worten / MediaMarkt / TuTrebol / Mercadona / Chafiras → /similar?site=… (backend: bySite фильтр в 3 запросах parse_products); над результатами подпись магазина.
- Место хранения: поля location_city (Дубай/Тенерифе), storage_place, storage_rack, storage_shelf (PUT whitelist + форма правки); фото/видео места хранения: storage_media jsonb [{url,kind,at}], POST/DELETE /api/items/:id/storage-media, превью в карточке с ✕.
- Миграция: supabase-migration-v186-home-items-storage.sql (5 колонок). build: 'v186-2026-09-12'.

## v187 (12.09.2026) — Tools: мультимагазин, полнопольный поиск, модалка результатов
- «Найти в магазинах ▾» — меню с ГАЛКАМИ (как в попапе расширения): отметить 1..N магазинов + кнопка «🔍 Найти (N) / везде»; ни одной = все. Выбор запоминается (itemShopSel).
- Результаты поиска — ВСПЛЫВАЮЩЕЕ ОКНО (itemSearchModal): шапка с фото предмета + списком магазинов, список совпадений с ценами; из карточки блок результатов убран. Backend /similar принимает sites=a,b,c (.in('site', [...])).
- Поиск по предметам (GET /api/items?q=) теперь по ВСЕМ полям: +category, notes, location_city, storage_place, storage_rack, storage_shelf; fallback на старый набор полей, если миграция v186 не выполнена.
- Поля места/стеллажа/полки в правке — input с datalist (dl-item-place/rack/shelf): подсказки из уже заполненных значений базы + ручной ввод.
- itemSimilar state удалён. build: 'v187-2026-09-12'.

## v188 (12.09.2026) — умное сравнение с каталогом (все 6 ходов)
1) Справочник каталога в промпт распознавания: rebuildCatalogHints() (кэш 6ч) — топ uni/bigram из parse_products.name + термины из ручных правок name_es; buildItemPrompt(hints) получает пункт 9 «используй формулировку из справочника».
2) normalizeBrand() к parse_brands; normalizeMpn(); /similar ищет MPN также по article.
3) CAT_ROOTS: категория предмета → корни разделов каталога (herramient/electric/ferreter/fontaner…), +4 к скорингу за свой раздел (COLS +category).
4) Эмбеддинги pgvector: embedText() — СНАЧАЛА локальный AI (env LOCAL_EMBED_URL, Ollama /api/embeddings, модель LOCAL_EMBED_MODEL=nomic-embed-text), затем gemini-embedding-001 (outputDimensionality 768). home_items.name_embed при recognize/rerecognize; POST /api/parse/embed-catalog (порциями ≤500); match_products RPC; /similar добавляет кандидатов match_by='embed'.
5) Атрибутная перепроверка топ-5 визуального поиска: buildAttrRankPrompt (цвет/форма/комплект/маркировка), финальный score = 0.55×визуал + 0.45×attr; фронт: бейдж 🧬 с заметками.
6) item_feedback: POST /api/items/:id/feedback {product_url, verdict} — «✕ не то» в модалке поиска; bad-url исключаются из /similar и /similar-visual; правки name_es/brand/mpn в PUT → verdict='correction' + сброс кэша справочника.
- Миграция: supabase-migration-v188-items-intel.sql (pgvector, name_embed, item_feedback, match_products). build: 'v188-2026-09-12'.
