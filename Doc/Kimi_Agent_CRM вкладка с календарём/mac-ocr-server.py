...
Поэтому ставим в отдельное окружение venv:
    ...
    python3 -m venv venv
    ./venv/bin/pip install ocrmac "markitdown[pdf]"

(markitdown нужен для чтения ТЕКСТОВОГО СЛОЯ цифровых PDF — /pdf-md;
 без него сервер работает как раньше, просто PDF идут через Vision OCR)
...
# v56: MarkItDown — цифровые PDF в Markdown по текстовому слою (без OCR).
# Пакет необязателен: нет — /pdf-md вернёт 503, фронт уйдёт в обычный Vision OCR.
try:
    from markitdown import MarkItDown
    _md = MarkItDown()
except ImportError:
    _md = None
    print("markitdown не установлен — PDF с текстовым слоем пойдут через Vision OCR.")
    print('Чтобы включить Markdown-режим: ./venv/bin/pip install "markitdown[pdf]"')
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith("/ocr") or self.path.startswith("/pdf-md"):
            self._json(405, {"error": "Используйте POST с файлом в теле запроса"})
        else:
            self._json(200, {"status": "ok", "service": "mac-ocr-server", "port": PORT, "markitdown": bool(_md)})

    def _handle_pdf_md(self):
        """v56: PDF → Markdown по текстовому слою (MarkItDown). Тело — сам PDF."""
        if _md is None:
            self._json(503, {"error": 'markitdown не установлен: ./venv/bin/pip install "markitdown[pdf]"'})
            return
        try:
            ...
            try:
                ...
                if len(text) < 40:
                    # скан без текстового слоя — фронт уйдёт в Vision OCR
                    self._json(422, {"error": "в PDF нет текстового слоя (скан) — нужен OCR"})
                    print(f"PDF-MD {name}: текстового слоя нет")
                    return
                self._json(200, {"text": text, "chars": len(text)})
                print(f"PDF-MD {name}: {len(text)} симв. markdown")
            finally:
                try:
                    os.unlink(tmp.name)
                except OSError:
            ...
            print(f"ERR pdf-md {e}")

    def do_POST(self):
        if self.path.startswith("/pdf-md"):
            self._handle_pdf_md()
            return
