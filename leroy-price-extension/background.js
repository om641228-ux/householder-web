...
// v1.23.0: fetch с таймаутом — без него зависший запрос к серверу вешал всю очередь навсегда
const fetchT = (url, opts = {}, ms = 25000) => {
...
function extractOnPage() {
  ...
  const __visualPrice = (root, anchorEl) => {
    ...
    const readPrice = (el) => { // → число или null
      ...
      if (/env[ií]o|gratis|pedidos?|entrega|recog(ida|er)|entregar/i.test(t)) return null; // v1.27.5: «Envío gratis en pedidos +29€» — доставка, НЕ цена
      if (/\+\s*\d/.test(t)) return null; // v1.27.5: «+29€» — порог бесплатной доставки, НЕ цена
...
async function collectOne(api, token, p) {
  ...
  if (failReason) {
    try {
      await fetchT(`${api}/api/parse/ext-price?token=${encodeURIComponent(token)}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
...
async function run(api, token, batch, mode, staleDays, continuous, site) {
  ...
  try {
    for (;;) {
      ...
      progress(tag + '⏳ Запрашиваю очередь у сервера…'); // v1.23.0: видно, что сбор жив на этапе запроса
      ...
      for (const p of items) {
        ...
        // v1.23.0: сторожевой таймаут на один товар — если вкладка/инъекция зависла, идём дальше
        const r0 = await Promise.race([collectOne(api, token, p), sleep(90000).then(() => ({ saved: false, failReason: 'timeout-90s' }))]);
        if (r0.saved) totalOk++;
...
function extractLinksOnPage() {
  ...
  const __visualPrice = (root, anchorEl) => {
    ...
    for (const el of root.querySelectorAll('span,div,p,strong,b,em,s,del,strike,sup,h2,h3,h4')) {
      ...
      if (/env[ií]o|gratis|pedidos?|entrega|recog(ida|er)|entregar/i.test(t)) continue; // v1.27.5: «Envío gratis en pedidos +29€» — доставка, НЕ цена
      if (/\+\s*\d/.test(t)) continue; // v1.27.5: «+29€» — порог бесплатной доставки, НЕ цена
